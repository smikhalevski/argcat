...
describe('parseArgs', () => {
  ...
  test('parses a mixed shortcuts', () => {
    ...
    expect(parseArgs(['-ba', 'aaa'], { flags: ['a'], keepShorthands: true })).toEqual({ a: true, b: [], '': ['aaa'] });
  });
  ...
  test('parses a shorthand with multiple values', () => {
    expect(parseArgs(['-f', 'bar', '-f', 'baz'], { shorthands: { f: 'foo' } })).toEqual({ foo: ['bar', 'baz'] });
  });

  test('parses multiple shorthands with a value', () => {
    expect(parseArgs(['-ab', 'bar'], { shorthands: { a: 'aaa', b: 'bbb' } })).toEqual({ aaa: [], bbb: ['bar'] });
  });
